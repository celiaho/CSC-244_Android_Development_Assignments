...
import edu.bhcc.cho.note_takingserverapp.models.Profile
...
class ProfileApiService(context: Context) {
     ...
     */
    fun getProfile(onSuccess: (Profile) -> Unit, onError: (com.android.volley.VolleyError) -> Unit) {
     ...
     * @param onError Callback function to handle an error during the request.
     */
    fun deleteProfile(onSuccess: () -> Unit, onError: (com.android.volley.VolleyError) -> Unit) {
        val url = "$baseUrl/profiles/me"
    ...
    fun searchProfiles(
        ...
        val jsonArrayRequest = JsonArrayRequest( // Use JsonArrayRequest since the server returns an array
